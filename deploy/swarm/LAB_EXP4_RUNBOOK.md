# 实验四：v0.6-pow 的 Swarm 部署手册

本文档对应分支 `feature/swarm-lab-exp4-replica-pow`。本分支以 `v0.6-pow` 为业务基线，只迁移 Docker Swarm 部署层，不重写 `TopLevelUpdate` 业务结构，也不替换原有副本节点攻击防御逻辑。
...
- `vp0` 保持正常主节点，不启用 byzantine。
- 默认恶意节点是副本节点 `vp1`、`vp4`、`vp8`，对应 10 节点、`f=3` 的一组三个副本作恶节点。
- 副本节点作恶路径仍使用原代码中的 `tryByzantineForCTX` / `tryByzantineForCNS`。
- PoW 防御路径仍使用原代码中的 `makeTXbyPow`，内部伪造请求会带上 `nonce` 和 `target` 字段。
...
git fetch origin
git checkout feature/swarm-lab-exp4-replica-pow
git pull origin feature/swarm-lab-exp4-replica-pow
```
...
```bash
FABRIC_PEER_IMAGE=fabric-dns-peer:swarm-exp4
...
把 `/tmp/fabric-dns-peer-swarm-exp4.tar` 拷贝到两台服务器，并分别导入：
...
两台机器都必须能看到 `fabric-dns-peer:swarm-exp4`。
...
docker service ps fabricdns_vp1
docker service ps fabricdns_vp4
docker service ps fabricdns_vp8
...
然后重新部署 stack，让恶意副本节点拿到 `CORE_DNS_CHAINCODEID`：

```bash
bash scripts/swarm/deploy-stack.sh
```

这是实验四必须步骤。原始 `v0.6-pow` 的恶意副本节点逻辑会在副本容器内部读取该值，再构造带 PoW 字段的伪造 `TopLevelUpdate` 交易。
...
注意：脚本发送的是普通 6 参数用户请求。恶意副本节点内部是否改写请求、是否带 PoW 字段，由 `v0.6-pow` 原代码决定，不在脚本中伪造 8 参数请求。
...
- `vp0` 环境变量中 `CORE_PBFT_GENERAL_BYZANTINE=false`。
- `vp1`、`vp4`、`vp8` 环境变量中 `CORE_PBFT_GENERAL_BYZANTINE=true`。
- 恶意副本节点环境变量中 `CORE_DNS_CHAINCODEID` 不为空。
- `query-top-level.sh` 能查询链码状态。
- 普通 `TopLevelUpdate` 请求能被提交并出块。
- 日志能看到原始 `v0.6-pow` 的副本节点作恶/PoW 防御路径，而不是脚本层重写的业务请求。
...
如果恶意副本节点没有触发原始实验四逻辑，检查：

```bash
docker service inspect fabricdns_vp1 --format '{{json .Spec.TaskTemplate.ContainerSpec.Env}}'
docker service inspect fabricdns_vp4 --format '{{json .Spec.TaskTemplate.ContainerSpec.Env}}'
docker service inspect fabricdns_vp8 --format '{{json .Spec.TaskTemplate.ContainerSpec.Env}}'
```
