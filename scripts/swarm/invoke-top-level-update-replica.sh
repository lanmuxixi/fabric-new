...
: "${REPLICA_ENTRY_ENDPOINT:?Set REPLICA_ENTRY_ENDPOINT to the published gRPC endpoint of the byzantine replica, for example 10.92.2.139:7151.}"
...
docker run --rm \
  -e CORE_PEER_ADDRESS="${REPLICA_ENTRY_ENDPOINT}" \
  "${FABRIC_PEER_IMAGE}" \
